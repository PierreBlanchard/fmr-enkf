...
%
% Loads the initial ensemble for the L40 model
%
...
%                 NERSC
%
% Purpose:        Loads the initial ensemble for L40 model
%
...
function [x_true, E, t] = model_generate(prm)
    ...
    if prm.n ~= n
        error(sprintf('\n  error: prm.n = %d; \"%s\": n = %d', prm.n, fname, n));
    end
  
    if prm.m > n_sample
        error(sprintf('\n  error: prm.m = %d; \"%s\": n_sample = %d', prm.m, fname, n_sample));
    end
